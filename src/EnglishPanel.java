...
public class EnglishPanel extends JPanel {
    ...
    Word    currentWord;    //Li Wen: 当前单词，默认为WordList的第0个单词。
    ...
    */
    public EnglishPanel(User thisuser){
        ...
        //接下来这句，可能会删除
        currentWord = WordList.elementAt(0);
        NewWord = null;
        curposition = 0;
        roundcounter = 0;
        capcounter = 3;
    ...
    public void initEnglishPanel(){
        ...
        //Li Wen: EnglishPages里各个页面布局
        {
            ...
            //MemoryPagePanel布局
            {
                ...
                MemWordText.setText(currentWord.getFrontSide());
                MemMeanText.setText(currentWord.getBackSide());
                MemSentenceText.setText(currentWord.getEgSentence()+"\n"+currentWord.getEgSentenceTrans());
        ...
        //Li Wen: 监听事件绑定
        {
                ...
                MemoryButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        if (i == WordList.size()){
                            ...
                            //把第一个没掌握的词加到正在背诵的单词列表。
                            currentWord = WordList.elementAt(curposition);
                            MemoryingWords.add(currentWord);
            ...
            //MemoryPagePanel
            {
                ...
                MemForgetButton.addActionListener(new ActionListener(){
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        currentWord.setForget(1);
                        roundcounter++;
                        //掌握的单词不需要拼写
                        MemoryingWords.remove(currentWord);
            ...
            //DictatePagePanel
            {
                ...
                DicCheckButton.addActionListener(new ActionListener(){
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        System.out.println("now the Word is: "+currentWord.getFrontSide());
                        ...
                        //若拼写正确
                            ...
                            //如果wrong的次数为0，就掌握它。
                            if (currentWord.isemptyWrong()){
                                currentWord.setForget(1);
                            }
                            //如果wrong的次数大于0，则之后还会背到这个单词，并清空拼错次数。
                            else {
                                currentWord.setForget(0);
                                currentWord.setWrong(0);
    ...
    public void getNextWord(int choice){
        ...
        //正在拼单词
            ...
            for (;flagcount < roundcounter;) {
                ...
                if(tempNextWord.isForget() == -1) {
                    currentWord = tempNextWord;
                    DicWordText.setText(currentWord.getFrontSide());
                    DicMeanText.setText(currentWord.getBackSide());
                    DicAccentText.setText(currentWord.getAccent());
                    return;
                }
                else{
                    flagcount++;
    ...
    public void toSpell(){
        if ((WordsOverFlag == 1) || (roundcounter == capcounter) ){
            if (MemoryingWords.size() != 0){
                ...
                curposition = 0;
                currentWord = MemoryingWords.elementAt(curposition);
                DicWordText.setText(currentWord.getFrontSide());
                DicMeanText.setText(currentWord.getBackSide());
                DicAccentText.setText(currentWord.getAccent());
